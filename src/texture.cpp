...
namespace VE
{
	...
	static auto getBufferObject = []() -> Buffers&
		{
			return Application::GetInstance()->m_VkBuffers;
		};
	...
	{
		VkCommandBuffer commandBuffer = getBufferObject().BeginSingleTimeCommands();
		...
		getBufferObject().EndSingleTimeCommands(commandBuffer);
	}
	...
	void Texture::CreateTextureImage()
	{
		...
		Buffers::CreateBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, m_StagingBuffer, m_StagingBufferMemory);
	...
	{
		m_TextureImageView = SwapChain::CreateImageView(m_TextureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, m_MipLevels);
	}
	...
	void Texture::CreateTextureSampler()
	{
		...
		if (vkCreateSampler(getDevice(), &samplerInfo, nullptr, &m_TextureSampler) != VK_SUCCESS)
		{
