...
namespace VE
{
	...
	static auto getDescriptorSetLayout = []() -> VkDescriptorSetLayout&
		{
			return Application::GetInstance()->m_VkDescriptors.GetDescriptorSetLayout();
		};
	static auto getMSAASamples = []() -> VkSampleCountFlagBits&
		{
			return Application::GetInstance()->m_VkBuffers.GetMSAASamples();
		};

	Pipeline::~Pipeline()
	{
		vkDestroyPipeline(getDevice(), m_GraphicsPipeline, nullptr);
	...
	void Pipeline::CreateGraphicsPipeline()
	{
		...
		if (vkCreateGraphicsPipelines(getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_GraphicsPipeline) != VK_SUCCESS)
		{
	...
	void Pipeline::CreateRenderPass()
	{
		...
		if (vkCreateRenderPass(getDevice(), &renderPassInfo, nullptr, &m_RenderPass) != VK_SUCCESS)
		{
